...
public class Main{
    ...
    public static void main(String[] args) throws IOException{
        ...
        while(test --> 0){
            ...
            if(k >= m){ //simple check (do it for all ahead elements bois)
                k = m-1;
            }

            for(int i = 0; i <= k; i++){
                //first let's make inner boundaries bois
                int y = m-k-1; //the lesser this is the better bois (inner circle take away)
  
                int left = i;
                int right = n - (k-i+1);

                // out.println(left + " " + right);
                //so we have now ranges babyyyyy!!!!
                //now make one more box
                int si = left + y;
                int ei = right;  

                int min = Integer.MAX_VALUE;
                while(y-- >= 0){
                    min = min(min, max(arr[si], arr[ei]));
                    si--;
                    ei--;
                }
