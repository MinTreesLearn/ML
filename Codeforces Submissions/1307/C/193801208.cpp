...
void solve()
{
    ...
    for (auto it = m.begin(); it != m.end(); it++)
    {
        ...
        for (; it1 != m.end(); it1++)
        {
            ...
            debug(it1->first);
            int count1 = 0;
            vector<int> &v2 = it1->second;
            debug(v2)

            for (int i = 0; i < v1.size(); i++)
            {
                auto at = lower_bound(v2.begin(), v2.end(), v1[i]);
                if (at != v2.end())
                {
                    int indx = (int)(at - v2.begin());
                    debug(indx)
                    count += (v2.size() - indx);
                }
            }
            ...
            {
                auto at = lower_bound(v1.begin(), v1.end(), v2[i]);
                if (at != v1.end())
                {
                    int indx = (int)(at - v1.begin());
                    count1 += (v1.size() - indx);
                }
            }
            ...
            ans = max(ans, max(count, count1));
        }

        int val = it->second.size();
        int val1 = (val * (val - 1ll)) / 2ll;
        ans = max(ans, max(val, val1));
    }
