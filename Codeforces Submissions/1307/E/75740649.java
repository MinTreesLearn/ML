...
public class E {
    ...
    private static int[] solve(int n, int m, int[] s, int[] f, int[] h) {
        var sleepLeft = new int[m + 1];
        var sleepRight = new int[m + 1];
        ...
        var bySweet2 = new int[n + 1][];
        for (int sweet : sweets) {
            bySweet2[sweet] = bySweet.get(sweet).stream().mapToInt(i -> i).toArray();
        }
        ...
        for (int first = 0; first <= m; first++) {
            var leftStart = 1;
            var leftEnd = sleepLeft[first];
            var rightStart = leftEnd + 1;
            var rightEnd = n;
            if (first != 0 && (sleepLeft[first] < leftStart || sleepLeft[first] > leftEnd)) {
                continue;
            }
            var cows = 0;
            var ways = 1L;
            for (int sweet : sweets) {
//            for (var entry : bySweet.entrySet()) {
//                var sweet = entry.getKey();
                var left = 0L;
                var right = 0L;
                var both = 0L;
                for (var i : bySweet2[sweet]) {
//                    for (var i : entry.getValue()) {

                    var t = 0;
                    if (sleepLeft[i] >= leftStart && sleepLeft[i] <= leftEnd) {
                        left++;
                        t++;
                    }
                    ...
                    if (sleepRight[i] >= rightStart && sleepRight[i] <= rightEnd) {
                        right++;
                        t++;
                    }
                    if (t == 2) {
