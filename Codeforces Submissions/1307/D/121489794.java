...
public class C{
    ...
    static class Task{
        ...
        public void solveB(){
            ...
            adj= new List[n];
            for(int i=0;i<k;i++) special[i]= in.nextInt()-1;
            ...
            bfs(0,true); bfs(n-1,false);
            assert d1[n-1]==d2[0];
            List<Pair> p= new ArrayList<>();
            for(int s: special){
                p.add(new Pair(d1[s],d2[s]));
            }
            Collections.sort(p);
            int[] suf= new int[k];
            suf[k-1]= p.get(k-1).y;
            for(int i=k-2;i>=0;i--){
                suf[i]= Math.max(suf[i+1],p.get(i).y);
            }
            int res= -1;
            for(int i=0;i<k-1;i++){
                res= Math.max(res, p.get(i).x+1+suf[i+1]);
            }
            res= Math.min(res,d1[n-1]);
            out.println(res);
            
        }

        private void bfs(int u, boolean useD1){
            ...
            while(!q.isEmpty()){
                int cur= q.poll();
                for(int v: (List<Integer>) adj[cur]){
                    if(visited[v]) continue;
                    visited[v]=true;
                    if(useD1) d1[v] = d1[cur]+1;
                    else d2[v]= d2[cur]+1;
                    q.add(v);
                }
            }
