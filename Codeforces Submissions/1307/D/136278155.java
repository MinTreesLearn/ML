...
//			 System.out.println("hi");
		 ...
		 Comparator<Integer> comp = new Comparator<Integer>() {
			...
			public int compare(Integer o1, Integer o2) {
				return Integer.compare(d1[o1], d1[o2]);
			}
		};
		Collections.sort(a , comp);
		 int ans = 0 , curr = dn[a.get(k-1)];
		 for(int i = k-2 ; i>=0 ; i--) {
			 ans = max(ans , d1[a.get(i)] + curr + 1);
			 curr = max(curr , dn[a.get(i)]);
		 }
		 ans = min(ans , d1[n]);
	 ...
	 static boolean pres;
	 static int[] bfs(int n , int ind) {
		 int[] dis = new int[n+1];
		 Set<Integer> set = new HashSet<>();
		 LinkedList<int[]> ll = new LinkedList<>();
		 ll.add(new int[]{ind , 0});
		 while(!ll.isEmpty()) {
			 int[] pa = ll.removeFirst();
			 if(set.contains(pa[0])) continue;
			 dis[pa[0]] = pa[1];
			 set.add(pa[0]);
			 for(int v:adj.get(pa[0])) {
				 if(s[v] && s[pa[0]]) pres = true;
				 if(set.contains(v)) continue;
				 ll.addLast(new int[] {v , pa[1] + 1});
			 }
		 }
		 return dis;
	 }
