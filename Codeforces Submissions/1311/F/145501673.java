...
//			sieveOfEratosthenes((int)1e9);
			...
			while(tc-->0)
			{
				...
				fwt forCnt=new fwt(n);
				fwt forSum=new fwt(n);
				
				long ans=0;
				Arrays.sort(arr);
				for(int i=0;i<n;i++)
				{
					int pos=unique.get(arr[i].y);
					long cnt=forCnt.getSum(pos);
					long sum=forSum.getSum(pos);
					
					forCnt.updateBIT(n, pos, 1);
					forSum.updateBIT(n, pos, arr[i].x);
					ans+=arr[i].x*cnt-sum;
				}
