...
// editorial: start w chain & move leaves up (-1 sum depth every move)

public class cf1311e {

    public static void main(String[] args) throws IOException {
        int t = ri();
        next:
        while (t --> 0) {
            int n = rni(), d = ni(), cur = n * (n - 1) / 2, lb = 0;
            for (int dep = 0, sz = 1, left = n; left > 0; ++dep, sz <<= 1) {
                int delta = min(left, sz);
                left -= delta;
                lb += dep * delta;
            }
            if (d < lb || cur < d) {
            ...
            int ans[] = new int[n - 1], dep[] = new int[n], chd_cnt[] = new int[n];
            for (int i = 1; i < n; ++i) {
                ans[i - 1] = dep[i] = i;
                chd_cnt[i - 1] = 1;
            }
            ...
            while (cur > d) {
                int leaf = -1, new_par = -1;
                for (int i = 1; i < n; ++i) {
                    if (!bad[i] && chd_cnt[i] == 0 && (leaf == -1 || dep[i] < dep[leaf])) {
                        leaf = i;
                    }
                }
                if (leaf == -1) {
                ...
                for (int i = 0; i < n; ++i) {
                    if (chd_cnt[i] < 2 && dep[i] == dep[leaf] - 2) {
                        new_par = i;
                        break;
                    }
                }
                ...
                --chd_cnt[ans[leaf - 1] - 1];
                ++chd_cnt[new_par];
                ans[leaf - 1] = new_par + 1;
                --dep[leaf];
                --cur;
            }
            prY();
