...
public class Sleeping_Schedule {
    ...
    private static void solve() {
        ...
        int ans = goodSleeps(0, 0, arr, n, h, l, r);
        fw.out.println(ans);
    }

    private static int goodSleeps(int i, int time, int[] arr, int n, int h, int l, int r) {

        if (i >= n) return 0;
        if (dp[i][time] != -1) return dp[i][time];

        int ff = (time + arr[i]) % h;
        int ss = (time + arr[i] - 1 + h) % h;
        int ans = Math.max((util(ff, l, r) ? 1 : 0) + goodSleeps(i + 1, ff, arr, n, h, l, r),
                (util(ss, l, r) ? 1 : 0) + goodSleeps(i + 1, ss, arr, n, h, l, r));
        return dp[i][time] = ans;
    }

    private static boolean util(int x, int l, int r) {
        return (x >= l && x <= r);
    }
