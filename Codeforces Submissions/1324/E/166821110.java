...
public class Sleeping_Schedule2 {
    ...
    private static void solve() {
        ...
        int n = fs.nextInt(), h = fs.nextInt(), l = fs.nextInt(), r = fs.nextInt();
        int[] arr = readIntArray(n);
        int[][] dp = new int[n + 1][h];
        for (int[] row : dp) Arrays.fill(row, iMin);
        dp[0][0] = 0;
        for (int i = 1; i <= n; i++) {
            int a = arr[i - 1];
            for (int j = 0; j < h; j++) {
                int inRange = (j >= l && j <= r) ? 1 : 0;
                int jj = (j - a + h) % h;
                if (dp[i - 1][jj] != iMin) {
                    dp[i][j] = inRange + dp[i - 1][jj];
                }
                jj = (j - a + 1 + h) % h;
                if (dp[i - 1][jj] != iMin) {
                    dp[i][j] = Math.max(dp[i][j], inRange + dp[i - 1][jj]);
                }
            }
        }
        int ans = 0;
        for (int j = 0; j < h; j++) ans = Math.max(ans, dp[n][j]);
        fw.out.println(ans);
    }
