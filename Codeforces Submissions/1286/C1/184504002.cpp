...
multiset<string> ask(int l, int r, int len){
	multiset<string> ans;
	cout << "? " << l << " " << r << endl;
	cout.flush();
	while(sz(ans) < len){
...
void solve(){
	...
	multiset<string> a1 = ask(1, n, n * (n + 1) / 2);
	multiset<string> a2 = ask(2, n, n * (n - 1) / 2);
	multiset<string> a3 = ask(1, 1, 1);
	string ans;
	for(auto x : a2) a1.erase(a1.find(x));
	vector<string> aa1(n + 1);
	for(auto x : a1) aa1[sz(x)] = x;
	vll alp(26);
	for(int i = 1; i <= n; i++){
		vll temp(26);
		for(char c : aa1[i]){
