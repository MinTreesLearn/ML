...
public class cf1299c_2 {

    public static void main(String[] args) throws IOException {
        int n = ri(), a[] = ria(n), hull[] = new int[n + 1], p = 0;
        ...
        hull[p++] = 0;
        for (int i = 1; i <= n; ++i) {
            // slope(hull[-1] - hull[-2]) > slope(i - hull[-2])
            // slope(b - a) = (pre[b] - pre[a]) / (b - a)
            // (pre[hull[-1]] - pre[hull[-2]]) / (hull[-1] - hull[-2]) > (pre[i] - pre[hull[-1]]) / (i - hull[-1])
            // (pre[hull[-1]] - pre[hull[-2]]) * (i - hull[-1]) > (pre[i] - pre[hull[-1]]) * (hull[-1] - hull[-2])
            while (p >= 2 && (pre[hull[p - 1]] - pre[hull[p - 2]]) * (i - hull[p - 1]) > (pre[i] - pre[hull[p - 1]]) * (hull[p - 1] - hull[p - 2])) {
                --p;
            }
            hull[p++] = i;
        }
        for (int i = 1; i < p; ++i) {
            double val = (double) (pre[hull[i]] - pre[hull[i - 1]]) / (hull[i] - hull[i - 1]);
            for (int j = hull[i - 1]; j < hull[i]; ++j) {
                prln(val);
            }
        }
        close();
