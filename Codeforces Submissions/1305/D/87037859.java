...
public class cf1305d {

    public static void main(String[] args) throws IOException {
        int n = ri(), sz[] = new int[n], par[] = new int[n];
        ...
        int centroid = 0, maxsz = 0, maxnei = -1;
        dfs(t, 0, -1, sz);
        for(int nei : t.get(0)) {
            if(sz[nei] > maxsz) {
                maxsz = sz[nei];
        ...
        assign(t, centroid, -1, par);
        boolean[] no = new boolean[n];
        while(true) {
            int u = -1, v = centroid;
            for(int nei : t.get(centroid)) {
                if(nei != par[centroid] && !no[nei]) {
                    if(u == -1) {
                        u = nei;
                        while(t.get(u).size() > 1) {
                            for(int unei : t.get(u)) {
                                if(unei != par[u]) {
                                    u = unei;
                                    break;
                                }
                        ...
                        v = nei;
                        while(t.get(v).size() > 1) {
                            for(int vnei : t.get(v)) {
                                if(vnei != par[v]) {
                                    v = vnei;
                                    break;
                                }
            ...
            if(u == -1) {
                ...
                flush();
                centroid = ri() - 1;
                while(u != centroid && u >= 0) {
                    no[u] = true;
                    u = par[u];
                }
                while(v != centroid && v >= 0) {
                    no[v] = true;
                    v = par[v];
                }
            }
    ...
    static void assign(List<List<Integer>> g, int i, int p, int[] par) {
        par[i] = p;
        for(int n : g.get(i)) {
            if(n != p) {
                assign(g, n, i, par);
    ...
    static void dfs(List<List<Integer>> g, int i, int par, int[] sz) {
        sz[i] = 1;
        for(int n : g.get(i)) {
            if(n != par) {
                dfs(g, n, i, sz);
