...
public class cf1292b {

    public static void main(String[] args) throws IOException {
        long x0 = rnl(), y0 = nl(), ax = nl(), ay = nl(), bx = nl(), by = nl(), xs = rnl(), ys = nl(), t = nl();
        long data[][] = new long[64][2];
        data[0][0] = x0;
        data[0][1] = y0;
        int ind = 0;
        while(data[ind][0] < LMAX / ax && data[ind][1] < LMAX / ay) {
            ++ind;
            data[ind][0] = data[ind - 1][0] * ax + bx;
            data[ind][1] = data[ind - 1][1] * ay + by;
        }
        int ans = 0;
        for(int i = 0; i < ind; ++i) {
            for(int j = i; j < ind; ++j) {
                long treq = dist(data[i][0], data[i][1], data[j][0], data[j][1]);
                long dist = min(dist(xs, ys, data[i][0], data[i][1]), dist(xs, ys, data[j][0], data[j][1]));
                treq += dist;
                if(t >= treq) {
                    ans = max(ans, j - i + 1);
                }
    ...
    static long dist(long x0, long y0, long x1, long y1) {
