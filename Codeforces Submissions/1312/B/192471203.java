...
public class Main {
	...
	public static String solve(String s , String s1 ) {
		for ( int i = 0; i < s1.length(); ++i ) {
			if ( s.charAt(i) == s1.charAt(i) || s.charAt(i) == '?' || s1.charAt(i) == '?' ) continue;
			else return "No";
		}
		return "Yes";
	}

	public static int bfs( int node , ArrayList<Integer> arr ) {
...
class Pair {
	...
	int second;
	StringBuilder list;
	Pair(int first , int second , StringBuilder list ) {
		this.first = first ;
		this.second = second;
		this.list = list;
	}
