        ...
        for(int size = 2;size<=n;size++){
            for(int i=0;i+size-1<n;i++){
                for(int j=i;j<(i+size-1);j++){
                    if(dp[i][j]==dp[j+1][i+size-1]){
                        if(dp[i][j]<MAX_VALUE){
                            dp[i][i+size-1] = dp[i][j]+1;
                        }
                    }
                }
                if(dp[i][i+size-2]==dp[i+size-1][i+size-1]){
                    if(dp[i][i+size-2]<MAX_VALUE){
                        dp[i][i+size-1] = dp[i][i+size-2]+1;
                    }
                }
            }
        }
