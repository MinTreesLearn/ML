...
#define assert(x) (x)
#endif
vector<pair<char, int>> g[1000001];
int f[1000001];
int a[1000001];
bool in_set[1000001];
...
public:
    ...
    segment_tree_t(int l, int r) : l(l), r(r), m((l + r) / 2), min_val(0), lazy(0) {
        if (l != r) {
    ...
    void update(const int& u, const int& v, const int& x) {
        if (l > v || r < u) return;
        if (u <= l && v >= r) {
            min_val += x;
            lazy += x;
    ...
    int get(const int& u, const int& v) {
        if (l > v || r < u) return 1e9;
        if (u <= l && v >= r) return min_val;
        down();
        return min(left->get(u, v), right->get(u, v));
    }

    void set(const int& u, const int& x) {
        if (l == r) {
            min_val = x;
        } else {
            down();
...
void dfs(int u, int h = 0) {
    if (u == 0) {
        ...
        if (in_set[u]) {
            tree->update(0, h - 1, 1);
            f[u] = min(f[u], tree->get(0, h - 1));
        }
    }
    tree->set(h, f[u] + in_set[u]);
    for (auto&& [c, v] : g[u]) {
        f[v] = f[u] + 1;
        dfs(v, h + 1);
    }
