...
ll xs,ys,t;
ll calc(v<v<ll>>& reachable , ll l , ll r )
{
  ll t1=abs(xs-reachable[l][1])+
  abs(ys-reachable[l][2]);
  ll t2=abs(reachable[r][1]-reachable[l][1])+
  abs(reachable[r][2]-reachable[l][2]);
  if(t1+t2<=t)return 1;
  
  t1=abs(xs-reachable[r][1])+
  abs(ys-reachable[r][2]);
  t2=abs(reachable[r][1]-reachable[l][1])+
  abs(reachable[r][2]-reachable[l][2]);
  if(t1+t2<=t)return 1;
...
void solve()
{
  ...
  v<v<ll>>reachable;//time x,y;
  pll pres=mp(x0,y0);
  while(1)
  {
    ll dis=abs(xs-pres.fr)+abs(ys-pres.sc);
    if(dis<=t)
    {
      reachable.pb({dis,pres.fr,pres.sc});
    }
    if(pres.fr<=(inf/ax) && pres.sc<=(inf/ay))
    {
      pres.fr=(ax*pres.fr)+bx;
      pres.sc=(ay*pres.sc)+by;
    }
    else break;
  }
  trace(reachable);
  ll l=1,r=sz(reachable),mid,ans=0;
  while(l<=r)
  {
    mid=(l+r)>>1;
    ll pos=0;
    for(i=0;i<sz(reachable);i++)
    {
      if((i+mid-1)<sz(reachable))
      {
        pos|=calc(reachable , i , i+mid-1);
      }
      else break;
    }
    if(pos)
