...
def solve(x0,y0,ax,ay,bx,by,xs,ys,t):
    
    #x(i) = ax * x(i-1) + bx, x(0) = 0
    #y(i) = ay * y(i-1) + by, y(0) = 0
    cx,cy = x0,y0
    data = [(cx,cy)]
    dist = [abs(cx-xs)+abs(cy-ys)]
    while cx <= t+xs and cy <= t+ys:
        cx = ax * cx + bx
        cy = ay * cy + by
        data.append((cx,cy))
        dist.append(abs(cx-xs)+abs(cy-ys))
    ...
    def check(pos):
        ...
        for i in range(pos-1,-1,-1):
            travel_time = abs(data[i+1][0]-data[i][0]) + abs(data[i+1][1]-data[i][1])
            time -= travel_time
            if time < 0:
                return ans
            ans += 1
        ...
        for i in range(pos+1,len(data)):
            if i == pos+1:
                travel_time = abs(data[0][0]-data[i][0]) + abs(data[0][1]-data[i][1])
            else:
                travel_time = abs(data[i-1][0]-data[i][0]) + abs(data[i-1][1]-data[i][1])
            time -= travel_time
            if time < 0:
