...
void solve() {
  ...
  ll ANS = 0;
  FOR(n, LIM + 1) ANS += CNT[n] * len(S[n]);

  vi tmp(LIM + 1);
  FastSet ss(LIM + 1);
  FOR(n, LIM + 1) if (CNT[n]) ss.insert(n);
  ...
  while (1) {
    // 進む方向、近づく個数
    {
      ...
      while (1) {
        n = ss.next(n + 1);
        if (n > LIM) break;
        if (len(S[n]) > p) tmp[S[n][p]] += CNT[n];
      }
    }

    int best_p = max_element(all(tmp)) - tmp.begin();
    ll cnt = tmp[best_p];
    ll add = (Q - cnt) - cnt;
    if (add >= 0) break;
    ANS += add;
    {
      ...
      while (1) {
        n = ss.next(n + 1);
        if (n > LIM) break;
        if (len(S[n]) > p) tmp[S[n][p]] = 0;
        if (len(S[n]) <= p || S[n][p] != best_p) {
          CNT[n] = 0;
          ss.erase(n);
        }
      }
