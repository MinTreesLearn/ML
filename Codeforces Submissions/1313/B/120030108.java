...
//        int tc = 1;
        for (int rep = 0; rep < tc; rep++) {
            ...
            long Y = sc.nl();
            long miny = 1;
            if (X+Y - N > 0) miny = X+Y-N+1;
            
            miny = Math.min(miny, N);
            long maxy = Math.min(N, X+Y-1);
            pw.println(miny + " " + maxy);
        }
    ...
    static long solve(long[][] LR, long[][] UD, int N, int M, int K, int i, int j) {
//        cache = new Long[N][M][K];
        return 0L;
    }
