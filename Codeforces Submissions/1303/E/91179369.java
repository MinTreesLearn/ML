...
public class cf1303e_2 {

    public static void main(String[] args) throws IOException {
        int tt = ri();
        next: while (tt --> 0) {
            char[] s = rcha(), t = rcha();
            int n = s.length, m = t.length, next[][] = new int[n + 1][26];
            fill(next[n], n);
            for (int i = n - 1; i >= 0; --i) {
                for (int j = 0; j < 26; ++j) {
            ...
            for (int match = 0; match < m; ++match) {
                int dp[][] = new int[match + 1][m - match + 1];
                for (int[] row : dp) {
                    fill(row, n);
                }
                dp[0][0] = -1;
                for (int i = 0; i <= match; ++i) {
                    for (int j = 0; j <= m - match; ++j) {
                        if (dp[i][j] < n) {
                            if (i < match) {
                                dp[i + 1][j] = min(dp[i + 1][j], next[dp[i][j] + 1][t[i] - 'a']);
                            }
                            if (j < m - match) {
                                dp[i][j + 1] = min(dp[i][j + 1], next[dp[i][j] + 1][t[match + j] - 'a']);
                            }
                        }
                    }
                }
                if (dp[match][m - match] < n) {
                    prY();
                    continue next;
                }
            }
