...
int main()
{
    ...
    test
    {
        ...
        vector<vector<int>> nextPos(s.size()+1, vector<int> (26, 1e9));
        for(int i=0; i<s.size(); i++)
        {
            for(int j=i; j<s.size(); j++)
                nextPos[i][s[j]-'a'] = min(nextPos[i][s[j]-'a'], j);
        }

        vector<vector<int>> dp(s.size()+2, vector<int> (s.size()+2));
        ...
        auto cal = [&](string a, string b)
        {
            ...
            for(int i=0; i<=a.size(); i++)
            {
                for(int j=0; j<=b.size(); j++)
                {
                    ...
                    if(i<a.size() && nextPos[lenS][a[i]-'a']<1e9)
                        dp[i+1][j] = min(dp[i+1][j], nextPos[lenS][a[i]-'a']+1);

                    if(j<b.size() && nextPos[lenS][b[j]-'a']<1e9)
                        dp[i][j+1] = min(dp[i][j+1], nextPos[lenS][b[j]-'a']+1);
                }
            }
        ...
        {
            if(cal(t.substr(0, i), t.substr(i, t.size()-i)))
            {
                poss = 1;
                break;
            }
