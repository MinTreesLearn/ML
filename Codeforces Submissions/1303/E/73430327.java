...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		t=sc.next().toCharArray();
			notmemo=new int[c.length+1][t.length+1][t.length+1];
		for (int i = 0; i < notmemo.length; i++) {
			for (int j = 0; j < notmemo[i].length; j++) {
				Arrays.fill(notmemo[i][j],-1);
			}
		}
    ...
    public static int dp(int i, int j, int idx) {
    if(i==c.length) {
    	if(idx==t.length)
return 0;
    	return (int) -1e9;
    }
    if(notmemo[i][j][idx]!=-1) {
    	return notmemo[i][j][idx];
    }
    int take=(int) -1e9;
    int take1=(int) -1e9;
    if(j<t.length&&c[i]==t[j]) {
    	take=1+dp(i+1,j+1,idx);
    }
    if(idx<t.length&&c[i]==t[idx])
    {
    take1=dp(i+1,j,idx+1);	
    }   
    int leave=dp(i+1,j,idx);
    return notmemo[i][j][idx]=Math.max(take1, Math.max(leave,take));
	...
	static void sieve(int N) // O(N log log N)
	{
		...
		for (int i = 2; i <= N; ++i) // can loop till i*i <= N if primes array is not needed O(N log log sqrt(N))
			if (isComposite[i] == 0) // can loop in 2 and odd integers for slightly better performance
			{
				...
				for (int j = i * 2; j <= N; j += i) { // j = i * 2 will not affect performance too much, may alter in
														// modified sieve
					isComposite[j] = 1;
