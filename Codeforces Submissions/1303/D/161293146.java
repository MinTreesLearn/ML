...
public final class CFPS {
	...
	public static void main(String[] args) {
		...
		for (int tc = 0; tc < t; tc++) {
			...
			long n = fr.nextLong();
			int m = fr.nextInt();
			long[] arr = fr.nextLongArray(m);
			
			// Observations: 
			
			// 1. The task is to determine the minimum number of splits required
			//    to fill up the bag completely.
			
			// 2. Possibility 1: DP problem
			//    Possibility 2: Greedy + Bitmask problem
			
			// 3. We can combine boxes for free.
			
			// 4. If some bit is ON in 'n', and is also present among the boxes,
			//    it is optimal to use it up then and there.
			
			CountMap<Long> powers = new CountMap<>();
			for (long l : arr)
				powers.putCM(l);
			...
			for (int lvl = 0; lvl < 60; lvl++) {
				...
				if ((n & shiftt) > 0 && powers.containsKey(shiftt)) {
					n ^= shiftt;
					powers.removeCM(shiftt);
				}
			}
			
			// 5. Starting from the smallest SET bit, we will try to determine, if
			//    some smaller bits can constitute to form it. What else will the 
			//    smaller bits be used for anyway?
			//    --
			//    If however, smaller bits don't fare, we will break the smallest value
			//    larger than the current bit level and continue for larger ON bits.
			...
			for (int lvl = 0; lvl < 60; lvl++) {
				...
				if ((n & shiftt) > 0) {
					...
					// here, we have to find a combination of smaller available bits
					// to (possibly) make up this bit
					
					// if this bit cannot be made from smaller bits, we will break the
					// smallest bit larger than it and use it
					
					CountMap<Long> used = new CountMap<>();
					long reqd = shiftt;
					
					for (long chk = reqd; chk > 0; chk /= 2) {
						// we will take up 'chk' until reqd remains non-negative
						int available = powers.getCM(chk);
						int take = (int) Math.min(available, reqd / chk);
					...
					if (reqd == 0) {
						// some bits can be combined to form 'lvl' bit
						
						n ^= shiftt;
						
						while (!used.isEmpty()) {
							Entry<Long, Integer> e = used.pollFirstEntry();
							int power = e.getValue();
							int alreadyPower = powers.getCM(e.getKey());
							
							powers.put(e.getKey(), alreadyPower - power);
						}
						
					} else {
						
						// we need to break some bit down
						
						reqd = shiftt;
						long got = -1;
						int numSteps = -1;
						
						for (long chk = reqd; chk < 1e9 + 1e9; chk *= 2) {
							if (powers.containsKey(chk)) {
								numSteps = logk((int) (chk / reqd), 2);
								got = chk;
								break;
							}
						}
						...
						// 'got' will be broken down to get reqd
						powers.removeCM(got);
						n ^= shiftt;
						
						// we also need to add to powers, the descendants of
						// got
						
						for (long desc = got / 2; desc >= reqd; desc /= 2) {
							powers.putCM(desc);
							numDivs++;
						}
					}
