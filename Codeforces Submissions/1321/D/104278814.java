...
class Problem {
    ...
    public void solve(int testNum) {
        ...
        List<List<Integer>> backEdges = new ArrayList<>();
        List<List<Integer>> edges = new ArrayList<>();
        for(int i = 0; i <= n; i++) {
            edges.add(new ArrayList<>());
            backEdges.add(new ArrayList<>());
        }
        for(int i = 0; i < m; i++) {
            int a = parser.parseInt();
            int b = parser.parseInt();

            List<Integer> edge = edges.get(a);
            edge.add(b);
            List<Integer> backEdge = backEdges.get(b);
            backEdge.add(a);
        }
        ...
        while(!queue.isEmpty()) {
            ...
            for(int i = 0; i < len; i++) {
                ...
                for(int next : backEdges.get(curr)) {
        ...
        for(int i = 0; i < k - 1; i++) {
            ...
            if(dists[curr] - 1 < dists[next]) {
                minV += 1;
            } else if(dists[curr] - 1 == dists[next]) {
                boolean is = false;
                for(int v : edges.get(curr)) {
                    if(v == next) continue;
                    if(dists[next] == dists[v]) {
                        is = true;
                        break;
                    }
                }
        ...
        System.out.println(minV + " " + (minV + maxV));
    }
