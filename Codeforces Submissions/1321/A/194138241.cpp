...
void solve() {
    ...
    vi v2(n);
    cin >> v1 >> v2;
    int hisAlone= 0, myAlone= 0, common= 0;
    int myScore= 0, hisScore= 0;
    for(int i= 0; i< n; ++i) {
    	if(v1[i]== 1 and v2[i]== 0) myAlone++;
    	else if(v1[i]== 0 and v2[i]== 1) hisAlone++;
    	else if(v1[i]== 1 and v2[i]== 1) common++; 
    }
    if(myAlone== 0) {
    	cout << -1;
    	return;
    }
    hisScore= hisAlone+common;
    myScore= common;
    int more= hisScore-myScore+1;
   	if(more<= myAlone) {
 	  	cout << 1;
 	  	return;
   	}
   	double one= double(more)/myAlone;
   	int two= more/myAlone;
   	if(one== two) cout << two;
   	else cout << two+1;
}	
