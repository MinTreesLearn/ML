...
public class weird_algrithm {
	 ...
	 */
	 ...
	 static interactor test;
	  ...
	  static long ways(int i, int m, int len, int n, Long [][] dp) {
		  if(i == n) return 1;
		  if(len == 2 * m) return 1;
		 
		  if(dp[i][len] != null) return dp[i][len];
		  return dp[i][len] = (ways(i, m, len + 1, n, dp) + ways(i + 1, m, len, n, dp)) % mod;
	  } 
		static void solve() throws IOException {
			int [] n = inputIntArr();
			long [][] dp = new long[n[0]][2 * n[1] + 1];
			...
			output.write(dp[dp.length - 1][dp[0].length - 1] + "\n");
		}
		/*1 2 3 4 5 6*/
