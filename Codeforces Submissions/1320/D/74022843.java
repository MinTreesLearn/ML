...
public class C1320D {
    ...
    public static void main(String[] args) {
        ...
        calcHash(t, n, power1, hashEven1, hashOdd1, count0, modulo1, r1);
        var hashEven2 = new long[n + 1][];
        var hashOdd2 = new long[n + 1][];
        calcHash(t, n, power2, hashEven2, hashOdd2, count0, modulo2, r2);
...
//                System.out.println(String.format("%d,%d,%d %d,%d", qi + 1, fp11, fp21, fp12, fp22));
    ...
    private static void calcHash(String t, int n, long[] power, long[][] hashEven, long[][] hashOdd, int[] count0,
            long modulo, long r) {
        var limit = r * modulo;
//        System.out.println(String.format("modulo=%d,r=%d", modulo, r));
        for (int len = 1; len <= n; len <<= 1) {
            ...
            for (int i = 0; i < n; i++) {
                if (t.charAt(i) == '0') {
                    var bitEven = (i & 1) + r / 2; // 对偶数开头子字符串来说，i是偶数在子字符串也是偶数位，用0表示；否则用1。
                    var bitOdd = r - bitEven; // 对奇数位开头的子字符串来说，i是偶数在子字符串是奇数位，用1表示；否则用0。
                    he = (he * r + bitEven) % modulo;
                    ho = (ho * r + bitOdd) % modulo;
                }
                if (i >= len && t.charAt(i - len) == '0') {
                    var bitEven0 = ((i - len) & 1) + r / 2;
                    var bitOdd0 = r - bitEven0;
