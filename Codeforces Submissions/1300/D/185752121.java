...
public final class CFPS {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int tc = 0; tc < t; tc++) {
			...
			int n = fr.nextInt();
			Coord[] poly = new Coord[n];
			for (int i = 0; i < n; i++)
				poly[i] = new Coord(fr.nextLong(), fr.nextLong(), i);
			...
			// i, i + 1 and (n/2)+i, (n/2)+i+1
			// --
			// have to be parallel and equal
			
			for (int i = 0; i < n - 1; i++) {
				int of = n / 2 + i, ofp1 = of + 1;
				if (of > n - 1 || ofp1 > n - 1) continue;
				
				Coord first = poly[i], second = poly[i + 1];
				Coord third = poly[of], fourth = poly[ofp1];
				
				// (first -> second) and (third -> fourth) must
				// be equal and parallel
				
				if (!eqAndPrll(first, second, third, fourth)) {
					NO();
					continue OUTER;
				}
			}
	...
	static boolean eqAndPrll(Coord first, Coord second, Coord third, Coord fourth) {
		long x1 = first.xi, y1 = first.yi, 
				x2 = second.xi, y2 = second.yi, 
				x3 = third.xi, y3 = third.yi, 
				x4 = fourth.xi, y4 = fourth.yi;
		
		long dist1 = (y2 - y1) * (y2 - y1) + (x2 - x1) * (x2- x1);
		long dist2 = (y4 - y3) * (y4 - y3) + (x4 - x3) * (x4 - x3);
		
		boolean is = ((y2 - y1) * (x4 - x3)) == ((x2 - x1) * (y4 - y3)) && dist1 == dist2;
		
		/*
		 * if (!is) out.println(first.idx + " " + second.idx + " " + third.idx + " " +
		 * fourth.idx);
		 */
