...
* T setBit(T,k,side)                                        //works only when n = int or long and k is always int **returns int or long**
* String setBitString(T,k,side)                             //works only when n = int or long and k is always int **returns String**
* int getSetBits(long n)									//Gives Number of sets bits in a number
...
public class A {
    ...
    {		
    	int n = sc.nextInt();int a[] = sc.getIntArray(br.readLine());
    	for(int x = 31;x >= 0;x--) {
    		int cnt = 0,last = -1;
    		for(int k = 0;k < n;k++) {
    			if((a[k]&1<<x) != 0) {
