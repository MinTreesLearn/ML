...
vector<int> a;
void search() {
	for(a.push_back(a.empty() ? 1 : a.back()); check(a, n, k); a.back()++) {
		cnt++;
		search();
	}
	a.pop_back();
...
int main() {
	...
	//sum of elements of f^3(x) < 65
	cin >> n >> k;
	if(k > 2) {
		search();
		cout << cnt << '\n';
	} else if(k == 2) {
		vector<vector<mint>> dp(n + 1, vector<mint>(300));
		dp[0][0] = 1;
		for(int x = n; x; x--) {
			for(int s = 0; s <= n; s++)
				for(int cnt = 0; s + (cnt + 1)*x <= n & cnt < 299; cnt++) {
					dp[s + (cnt+1)*x][cnt + 1] += dp[s][cnt];
				}
		}
		mint ans = -1;
