...
public class cf1304e_3 {

    static int euler[], first[], dep[], p = 0;
    static sptb rmq;
    ...
    public static void main(String[] args) throws IOException {
        ...
        dfs(g, 0, -1);
        rmq = new sptb(euler, (a, b) -> first[a] < first[b] ? a : b);
        int q = ri();
        while (q --> 0) {
    ...
    static int lca(int a, int b) {
        return rmq.qry(min(first[a], first[b]), max(first[a], first[b]) + 1);
    }
    ...
    static class sptb {
        ...
        sptb(int[] a, IntOperator operator) {
            n = a.length;
            lg = new int[n + 1];
            int pow2 = 4;
            ...
            for (int j = 1; (1 << j) <= n; ++j) {
                for (int i = 0; (i + (1 << j)) <= n; ++i) {
                    lookup[i][j] = op.merge(lookup[i][j - 1], lookup[i + (1 << (j - 1))][j - 1]);
                }
            }
        }
        ...
        int qry(int l, int r) {
            int j = lg[r - l];
            return op.merge(lookup[l][j], lookup[r - (1 << j)][j]);
        }
    }

    static void dfs(List<List<Integer>> g, int i, int par) {
        if (par == -1) {
            dep[i] = 0;
        } else {
            dep[i] = dep[par] + 1;
