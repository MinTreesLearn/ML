...
public class Main {
    ...
    static int parent[],euler_tour_depth[],euler_tour[],first[],depth[];
    static seg_tree st;
    ...
    public static boolean solve(int x,int y,int a,int b,int k) {
        int dist1=get_dist(a,b);
        int dist2=Math.min(get_dist(x,a)+get_dist(y,b),get_dist(y,a)+get_dist(x,b))+1;
//        System.out.println(dist1+" "+dist2);
        if(k<Math.min(dist1,dist2)) {
            return false;
        }
        if(k>=dist1 && k%2==dist1%2) {
            return true;
        }
        if(k>=dist2 && k%2==dist2%2) {
            return true;
        }
        return false;
    }
    public static int get_dist(int u,int v) {
        int lca=get_lca(u,v);
        return Math.abs(depth[lca]-depth[u])+Math.abs(depth[lca]-depth[v]);
    }
