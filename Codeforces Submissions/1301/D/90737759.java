...
public class cf1301d {
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (k > 4 * n * m - 2 * n - 2 * m) {
            ...
            if (m > 1) {
                if (k > m - 1) {
                    ans.add(new step(m - 1, "R"));
                    k -= m - 1;
                } else if(k > 0) {
                    ans.add(new step(k, "R"));
                    k = 0;
                }
                if (k > m - 1) {
                    ans.add(new step(m - 1, "L"));
                    k -= m - 1;
                } else if(k > 0) {
                    ans.add(new step(k, "L"));
                    k = 0;
                }
            }
            if (k >= 1) {
            ...
            for (int i = 1; i < n; ++i) {
                if (m > 1) {
                    if (k > 3 * (m - 1)) {
                        ans.add(new step(m - 1, "RUD"));
                        k -= 3 * (m - 1);
                    } else {
                        if (k > 3) {
                            ans.add(new step(k / 3, "RUD"));
                            k %= 3;
                        }
                        if (k > 0) {
                            ans.add(new step(1, "RUD".substring(0, k)));
                            k = 0;
                        }
                    }
                    if (k > m - 1) {
            ...
            if (k > n - 1) {
                assert false;
                ans.add(new step(n - 1, "U"));
                k -= n - 1;
            } else if (k > 0) {
                ans.add(new step(k, "U"));
            }
            prln(ans.size());
            for (step s : ans) {
                pr(s.n);
