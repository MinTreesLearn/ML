...
void ok_boss()
{
    ...
    ll tot_moves=4ll*n*m - 2ll*n - 2ll*m;
    ...
    vector<pair<ll,char>> needed;

    f(i,1,m)
    {
        if(n-1)
        {
            needed.pb({min(moves,n-1),'D'});
            moves-=min(moves,n-1);
