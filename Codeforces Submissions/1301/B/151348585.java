    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (T-- > 0) {
            ...
            for(int i=1;i<n;i++){
                ...
                knownGap=Math.max(knownGap,Math.abs(cur-pre));
                pre=cur;
            }
            long num=(max-min)/2+min;
            long res=Math.max(knownGap,Math.max(num-min,max-num));
            result.append(res+" "+num+"\n");
        }
        System.out.println(result);
        System.out.close();
    }
    public static boolean solve(int a[],int ans,int index){
        int sum = 0;
        int l = 0;
        int r = 0;
        while(l<=r){
            if(sum==ans&&r-l>=2)return true;
            if(sum>ans){
                sum-=a[l++];
            }else{
                if(r==a.length)break;
