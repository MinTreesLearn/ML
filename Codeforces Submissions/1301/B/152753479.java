...
public class MotaracksBirthday {
    ...
    private static void solve() {
        ...
        int n = fs.nextInt();
        Integer[] arr = readIntArray(n);
        int max = iMin, min = iMax;
        for (int i = 0; i < n; i++) {
            if (arr[i] == -1) continue;
            boolean legal = (i - 1 >= 0 && arr[i - 1] == -1) || (i + 1 < n && arr[i + 1] == -1);
            if (legal) {
        ...
        int[] result = new int[]{(int) (2e9 + 100), -1};
        int temp = (max + min) / 2;
        util(max, min, temp, result);
        util(max, min, Math.max(0, temp - 1), result);
        util(max, min, temp + 1, result);
        for (int i = 0; i < (n - 1); i++) {
            if (arr[i] == -1 || arr[i + 1] == -1) continue;
            result[0] = Math.max(result[0], Math.abs(arr[i] - arr[i + 1]));
        }

        fw.out.println(result[0] + " " + result[1]);
    }

    private static void util(int max, int min, int x, int[] result) {

        int diff = Math.max(Math.abs(max - x), Math.abs(min - x));
        if (result[0] > diff) {
            result[0] = diff;
            result[1] = x;
        }
