...
struct Mono {
  using value_type = pair<vc<mint>, int>;
  using X = value_type;
  static X op(X x, X y) {
    if (x.se == -1) return y;
    if (y.se == -1) return x;
    ...
    FOR(i, 4) FOR(j, 4) {
      int k = (i & 2) + (j & 1);
      int s = 2 * (i & 1) + ((j & 2) >> 1);
      if (x.se & 1 << s) a[k] += x.fi[i] * y.fi[j];
    }
    return {a, y.se};
  }
  static X unit() { return {vc<mint>(), -1}; }
...
void solve() {
  ...
  ・3: →→
  */
  // 衝突時刻 (num,den)、タイプ、idx
  vc<tuple<ll, ll, ll, ll>> event;
  FOR(i, N - 1) {
    ll d = X[i + 1] - X[i];
    event.eb(d, V[i] + V[i + 1], 2, i);
    if (V[i] < V[i + 1]) { event.eb(d, (V[i + 1] - V[i]), 0, i); }
    if (V[i] > V[i + 1]) { event.eb(d, (V[i] - V[i + 1]), 3, i); }
  }
  ...
  auto make_seg_data = [&](int i) -> T {
    vc<mint> x = {mint(1) - P[i], mint(0), mint(0), P[i]};
    return {x, CAN[i]};
  };

  vc<T> seg_raw(N);
  FOR(i, N) seg_raw[i] = make_seg_data(i);
  ...
  for (auto&& [num, den, t, idx]: event) {
    mint time = mint(num) / mint(den);
    assert(CAN[idx] & 1 << t);
    CAN[idx] ^= 1 << t;
    seg.set(idx, make_seg_data(idx));
    res.eb(time, calc());
  }
  // 最後まで生きる確率は除外
  mint p = res.back().se;
  for (auto&& [t, q]: res) q -= p;
