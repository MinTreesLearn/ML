...
ll helper(ll index,ll op,ll ep,ll prev) {
    if(index==n)return 0;
    if(dp[prev][index][op][ep]!=-1){
        return dp[prev][index][op][ep];
    }
    if(cur[index]!=0){
        return dp[prev][index][op][ep]=(prev!=cur[index]%2)+helper(index+1,op,ep,cur[index]%2);
    }
    ll ecase=n+5;
    ll ocase=n+5;
    if(ep>0){
        ecase=(prev==1)+helper(index+1,op,ep-1,0);
    }
    if(op>0){
        ocase=(prev==0)+helper(index+1,op-1,ep,1);
    }
    return dp[prev][index][op][ep]=min(ocase,ecase);
}
...
void solve(ll TC) {
    ...
    cout<<helper(0,p[1],p[0],cur[0]==0?2:cur[0]%2)<<endl;
}
