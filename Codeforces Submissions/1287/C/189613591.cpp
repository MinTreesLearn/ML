...
ll dp[105][105][105][5], arr[105], n;

ll dfs(ll cur, ll cnt_odd, ll cnt_even, ll prev){
    if(cur >= n) return 0ll;
    if(dp[cur][cnt_odd][cnt_even][prev] != -1) return dp[cur][cnt_odd][cnt_even][prev];   

    if(arr[cur]){
        return dp[cur][cnt_odd][cnt_even][prev] = dfs(cur + 1, cnt_odd, cnt_even, arr[cur] % 2) + 
        ((prev) != (arr[cur] % 2)) - (prev == 2);
    }   

    ll a1 = 1e9, a2 = 1e9;

    if(cnt_odd > 0){
        a1 = dfs(cur + 1, cnt_odd - 1, cnt_even, 1) + (prev == 0);
    }
    if(cnt_even > 0){
        a2 = dfs(cur + 1, cnt_odd, cnt_even - 1, 0) + (prev == 1);
    }
...
signed main(){
    ...
    ll ans = dfs(0, cnt_odd, cnt_even, 2);
