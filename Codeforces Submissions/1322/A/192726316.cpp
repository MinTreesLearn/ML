...
int main(){
int n,res=0;cin>>n;string s;cin>>s;int opn=0,clo=0,cto=-1,ctc=-1;
f(n){  if(s[i]=='(')opn++;else clo++;}
if(opn==clo){
f(n){
  if(s[i]=='(')opn++;else clo++;
  if(clo>opn&&cto==-1){cto=0;ctc=1;clo=1;opn=0;}
  if(cto!=-1&&(clo==opn)){res+=(2*clo);
//   cout<<clo<<" ";
  cto=-1;ctc=-1;clo=0;opn=0;}
}
cout<<res<<endl;
}
else cout<<-1<<endl;
