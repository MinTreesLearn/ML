...
// https://codeforces.com/contest/1322/submission/72635098

public class cf1322b_2 {

    public static void main(String[] args) throws IOException {
        int n = ri(), a[] = ria(n), ans = 0;
        if(n % 2 == 0){
            for(int v : a){
                ans ^= v;
            }
        ...
        for (int i = 0; i < 25; ++i) {
            ...
            tail = radix_sort(tail);
            int p = n - 1, x = 0;
            for (int j = 0; j < n - 1; ++j) {
                while (j < p && tail[j] + tail[p] >= 1 << i) {
                    --p;
                }
                x ^= (n - 1) - max(j, p);
            }
    ...
    public static int[] radix_sort(int[] f) {
        int n = f.length;
        int[] to = new int[n];
        {
            int[] b = new int[65537];
    ...
    /* static int[] radix_sort(int[] a) {
        int[] sorted = new int[a.length], cnt = new int[0x10001];
