...
void update(int cc, int lt, int rt, int px, int pp) {
	...
	int mt = (lt + rt) >> 1;
	if (px <= mt) {
		update(cc << 1, lt, mt, px, pp);
	}
	else {
		update(cc << 1 | 1, mt + 1, rt, px, pp);
	}
...
void just_do_it() {
	...
	for (int i = 1; i <= n; i++) {
		...
		if (l2 == r2 && l2 > 1 && r2 < n) {
			...
			update(1, 1, n, r1 - l1 + 1, -1);
			s.erase(l2);
			//len.erase(len.find(r2 - l2 + 1));
			update(1, 1, n, r2 - l2 + 1, -1);
			s.erase(l3);
			//len.erase(len.find(r3 - l3 + 1));
			update(1, 1, n, r3 - l3 + 1, -1);
			//len.insert(r3 - l1 + 1);
			update(1, 1, n, r3 - l1 + 1, 1);
		}
		else {
			//len.erase(len.find(r2 - l2 + 1));
			update(1, 1, n, r2 - l2 + 1, -1);
