...
ll n, m;
vecpair edge;
vecvec(sec, 5001, 5001);
vector<ll> adj[5001];
vector<vector<ll>> query;

bool cmp(const vec &a, const vec &b)    {return a[2] < b[2];}
...
int main()
{
    ...
    cin >> n;
    for(ll i = 1, u, v; i < n; i ++)       cin >> u >> v, adj[u].pb(v), adj[v].pb(u), edge.pb({u, v});
    ...
    sort(all(query), cmp);
    for(auto &x : sec)  for(auto &y : x)    y = INF;

    for(ll i = 0; i < m; i ++)
    {
        vec p(5001, 0);
        ll s = query[i][0], e = query[i][1], b = query[i][2];
    ...
    if(yes)
        for(auto x : edge)
            cout << min((ll)1e6, sec[x.first][x.second]) << sp;
    else    cout << -1 << endl;
}
