...
public class Main {
	...
	boolean f=true;
	PriorityQueue<book> pqx=new PriorityQueue<>((x1,y1)->x1.idx>y1.idx? 1:-1);
	
	for (int i = 0; i < z.length; i++) {
		pqx.add(new book(x[i], z[i]));
	}
	...
	while (!pqx.isEmpty()) {
			...
			int prev=pqx.peek().idx;
			
			while(!pqx.isEmpty()&&prev==pqx.peek().idx) {
				pq.add(new book(pqx.peek().idx, pqx.poll().score));
			
			}
//			int curo=pqx.peek().idx;

			
			int count=pq.size()-1;
			
			while((pqx.isEmpty()||pqx.peek().idx>prev)&&pq.size()>0) {
			book now=pq.poll();
			cost+=(prev-now.idx)*1l*now.score;
