...
public class Template {
	...
	public static void main(String[] args){
        ...
        while (yo-- > 0) {
            int n = sc.nextInt();
            long[] a = sc.readLongs(n);
            long[] b = sc.readLongs(n);
            Map<Long,PriorityQueue<Long>> map = new TreeMap<>();
            for(int i = 0; i < n; i++){
                map.computeIfAbsent(a[i], x -> new PriorityQueue<>());
                map.get(a[i]).add(b[i]);
            }
            ...
            for(int i = 0; i < n; i++){
                ...
                long add = 1l;
                while(size != 1){
                    if(!map.containsKey(a[i]+add)){
                        map.computeIfAbsent((a[i]+add), x -> new PriorityQueue<>()); 
                    }
                    else {
                        size += map.get(a[i]+add).size();
                    }
                    set.add(a[i]+add);
                    add++;
                    size--;
            ...
            for(long e : map.keySet()){
                PriorityQueue<Long> two = map.get(e);
                while(!two.isEmpty()){
                    long e1 = two.poll();
                    sum += e1;
                    pq.add(e1);
