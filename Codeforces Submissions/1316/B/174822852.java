...
public class B_String_Modification {
    ...
    public static void solve(FastReader f, PrintWriter out) {
        ...
        for(int i = 2; i <= n; i++) {
            ...
            curr.append(s.substring(i-1));
            if((n-i+1)%2 == 0) {
                curr.append(s.substring(0, i-1));
            } else {
                curr.append(new StringBuilder(s.substring(0, i-1)).reverse());
            }

            if(curr.compareTo(ansString) < 0) {
                ansString = curr;
                ansK = i;
            }
        }
