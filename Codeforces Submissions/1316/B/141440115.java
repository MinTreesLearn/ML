...
public class Main {
    ...
    public static void main(String args[])throws Exception{
        ...
        for(int f=0;f<t;f++){
            ...
            lst.add(str);
            StringBuilder sin=new StringBuilder(str.charAt(0)+"");
            StringBuilder sap=new StringBuilder(str.charAt(0)+"");
            ArrayList<String> fin=new ArrayList<>();
            fin.add(str);
            for(int i=1;i<n;i++){

                if((n-i+1)%2==0){
                    lst.add(str.substring(i)+sin);
                }else
                {
                    lst.add(str.substring(i)+sap);
                }
                sap.append(str.charAt(i)+"");
                sin.insert(0,str.charAt(i)+"");
                fin.add(lst.get(lst.size()-1)+"");
            }
            Collections.sort(lst);
            int ind=0;
            for(int i=0;i<n;i++){
                if(lst.get(0).equals(fin.get(i))){
                    ind=i;
                    break;
                }
            }
