     ...
     static class Combinations{
	    	 ...
	    	 long[] inf;
	    	  public Combinations(long N , long mod) {
				...
				inf = new long[(int)N+1];
				fac[0] = 1;
				for(int i =1 ; i<=N ; i++) fac[i] = (fac[i-1]*i)%mod;
			    long[] naturalNumInverse = new long[(int)N+1];
				naturalNumInverse[0] = naturalNumInverse[1] = 1;
			    for (int i = 2; i <= N; i++)
			        naturalNumInverse[i] = naturalNumInverse[(int)(mod % i)] * (mod - mod / i) % mod;
			    
			    inf[0] = inf[1] = 1;
			    
			    for (int i = 2; i <= N; i++)
			        inf[i] = (naturalNumInverse[i] * inf[i - 1]) % mod;
			}
	    	  
	    	  long ncr(long N, long R, long mod)
	    	  {		if(R<0 || R>N ) return 0;
	    		    long ans = ((fac[(int)N] * inf[(int)R])
	    		              % mod * inf[(int)(N - R)])
	    		             % mod;
	    		    return ans;
	    		}
...
/***
	...
	c3 = (a0*b3 + a1*b2 + a2*b1 ) | c4 = (a1*b3 + a2*b2) | c5 = (a2*b3)
