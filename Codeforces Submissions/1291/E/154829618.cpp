...
void yes(bool t = 1) { print(t ? "yes" : "no"); }
void no(bool t = 1) { yes(!t); }
#line 1 "/home/maspy/compro/library/ds/weightedunionfind.hpp"
...
struct WeightedUnionFind {
  ...
  WeightedUnionFind(int N) : N(N), vals(N, Group::unit()), size(N, 1) {
    par.resize(N);
    iota(all(par), 0);
  }
...
#line 1 "/home/maspy/compro/library/alg/group_xor.hpp"
...
void solve() {
  ...
  vvc<int> edges(N + 1);
  /*
  「必ず 0」を表す点 0 をつける。K + 1 頂点。
  */
  WeightedUnionFind<Group_Xor<int>> uf(K + 1);
  FOR(k, K) {
    LL(sz);
    FOR_(sz) {
      LL(x);
  ...
  // 各連結成分に対して、(0 の個数, 1 の個数) を持たせておく
  // sum = sum of min(x0, x1) も管理する
  vc<pi> dat(K + 1, {1, 0});
  ll sum = 0;

  auto add_edge = [&](int a, int b, int x) -> void {
    auto [ra, xa] = uf.get(a);
    auto [rb, xb] = uf.get(b);
    x = x ^ xa ^ xb;
    a = ra, b = rb;
    ...
    uf.merge(a, b, x);
    if (uf.get(a).fi == b) { swap(a, b); }
    // 根が a となるようにマージした
    // dat[b] を dat[a] に足しこむ
    auto [a0, a1] = dat[a];
    auto [b0, b1] = dat[b];
    sum -= min(a0, a1) + min(b0, b1);
    if (x == 0) {
      dat[a] = {a0 + b0, a1 + b1};
    } else {
      dat[a] = {a0 + b1, a1 + b0};
    }
    sum += min(dat[a].fi, dat[a].se);
  };
