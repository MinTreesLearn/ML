...
void solve(int cake)
{
    ...
    fi(i, 0, n)
    {
        ...
        if (!flag)
            temp1.push_back({maa, mii, flag});
        else
        {
            temp2.push_back({maa, mii, flag});
        }
    }
    svi(temp1);
    vector<int> tt;
    fi(i, 0, (int)temp1.size()) tt.push_back(temp1[i][0]);
    svi(tt);

    fi(i, 0, (int)temp1.size())
    {
        ans += tt.end() - upper_bound(tt.begin(), tt.end(), temp1[i][1]);
    }
    int t = temp1.size();
    fi(i, 0, (int)temp2.size())
    {
        ans += n;
        ans += t;
    }
